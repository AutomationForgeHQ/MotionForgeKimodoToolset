...
public:
	...
	UKimodoSkill()
	{
		...
		Instructions = TEXT(
			...
			"\n"
			"Two limits decide when to use something else. A beat is at most ten seconds - a longer "
			"clip is more beats, up to sixty seconds in all. And it "
			...
			"downloading. Generating starts a stopped local runner by itself, Docker first, and waits "
			"for the text encoder, so a stopped container is not a reason to call anything first. Only "
			"a missing Docker needs the user.\n"
			"\n"
			"Warn the user before the first setup on a machine. It downloads roughly 20GB and can "
			"take a long time. It also needs a Hugging Face token stored on the Keys page and the "
			"Llama 3 licence accepted for that token, because the text encoder is a gated model. The "
			"two are separate facts: MotionForge's setup steps ask Hugging Face whether the token has "
			"been granted and say so, and a token without the grant installs fine and fails every "
			"generation. Only the user can request access, and the review can take hours.\n"
			"\n"
			...
			"else.\n"
			"\n"
			"One failure names its own fix and should be believed: a runner that predates the job "
			"contract. It says so, at the status and again on any job it finishes, because it "
			"reports the old vocabulary and cannot report an artifact route or a VRAM figure. "
			"Rebuild it - locally with the setup tool, or by restarting a rented pod so it pulls "
			"the current image. Nothing else will make it work, and the old image is still on the "
			"machine under its own tag, so a rebuild is the only thing that replaces it.\n"
			"\n"
			"Otherwise, do not rebuild the container as a general remedy. It is the fix for exactly "
			"three things: an Image State of Drifted on the status, meaning the built image came from "
			...
			"whoever owns that machine has to start it. A rented GPU is remote too, and it bills by "
			"the hour while it is up whether or not anything generates: say so when one is running, "
			"and do not leave one up when the work is done.");
	}
};
